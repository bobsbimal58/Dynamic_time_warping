...
public class InsertData extends AppCompatActivity implements SensorEventListener,View.OnClickListener {
    ...
    public ArrayList<Float>warpingDistance=new ArrayList<>();
    public ArrayList<Double> displacementArray= new ArrayList<Double>();
    public ArrayList<Float>mapDataCopy= new ArrayList<>();
    public Cursor cur;
    float data;
    Integer counter=0;
    Float velocity=1.1f;
    ...
    //for smallest warping distance
    int index=0;
    public HashMap<Double, Float> location = new HashMap<Double, Float>();
...
*/
        ...
        if (isInsert) {
            ...
            DBHelper.getInstance().insert(0, 0, 0, xaxis, yaxis, zaxis, average, calculateStandarddeviation(sensorData), 0, 0, 0, degreeDisplay);
        ...
        if (calcdiff) {
            ...
            if (cur.isLast() == false) {
                ...
                while (cur.moveToNext()) {
                    ...
                    Toast.makeText(getApplicationContext(), "pos:" + curPos, Toast.LENGTH_SHORT).show();
                    time = curPos * 0.5;
                    displacement = time * velocity;
                    displacementArray.add(displacement);
                    magnetismd.setText(String.valueOf(displacementArray));
                    location.put(displacement, average_database);
                    //magnetismx.setText(String.valueOf(location));
                    mapDataCopy.add(average_database);
                    Collections.reverse(mapDataCopy);
                    magnetismy.setText(String.valueOf(mapDataCopy));
            ...
            if (testDataFinal.size() > 2) {
                if (counter < (mapData.size() - 2)) {
                    ...
                    if (testDataFinal.size() > 2) {
                        ...
                        warpingDistance.add(Float.valueOf(String.valueOf((dtw.getDistance()))));
                        magnetismy.setText(String.valueOf(warpingDistance));
                ...
                if (warpingDistance.size() != 0) {
                    //for getting smallest warping distance
                    float smallest = warpingDistance.get(0);

                    for (int i = 1; i < warpingDistance.size(); i++) {
                        if (warpingDistance.get(i) < smallest) {
                            smallest = warpingDistance.get(i);
                            index = i;
                            magnetismx.setText(String.valueOf(warpingDistance));
                        }
                    }
                } else {
                    Toast.makeText(getApplicationContext(), "please collect test or map data first", Toast.LENGTH_SHORT).show();
                }


                if (mapDataCopy.size() >= index) {
                    Object val = mapDataCopy.get(index);


                    if (location.size() != 0) {
                        for (Iterator iter = location.entrySet().iterator(); iter.hasNext(); ) {
                            Map.Entry e = (Map.Entry) iter.next();
                            if (val.equals(e.getValue())) {
                                Object key = e.getKey();
                                magnetismx.setText(String.valueOf(key));
